...
<ResponseField name="socialUrls" type="object">
   Extracted social/contact URLs:
   - `emailGeneral` - Emails found
   - `phoneGeneral` - Phone numbers
   - `linkedinProfile` / `linkedinCompany` - LinkedIn URLs
   - `facebookProfile` / `instagramProfile` / `twitterUser` - Social profiles
   - `youtubeChannel` / `tiktokProfile` - Video platforms
</ResponseField>
